 ...
 *
 * La primera version solo probaba apaisado, y por eso no detecto que en una
 * columna estrecha el velo llegaba a cubrir el 95 por ciento del lienzo y la
 * densidad de particulas caia a un tercio. Toda geometria nueva que el sitio
 * pueda adoptar tiene que entrar aqui.
 */
const GEOMETRIAS = [
  { nombre: 'apaisado', ancho: 640, alto: 320 },
  { nombre: 'vertical', ancho: 380, alto: 900 },
]
const INK = [42, 59, 76]
...
/** Bloques de texto tipicos de la seccion de bio, por geometria. */
function cajasDe(g) {
  const mx = g.ancho < 620 ? g.ancho * 0.06 : g.ancho * 0.24
  const x0 = mx
  const x1 = g.ancho - mx
  const n = g.ancho < 620 ? 5 : 3
  const util = g.alto * 0.82
  const paso = util / n
  const cajas = []
  for (let i = 0; i < n; i++) {
    cajas.push({
      x0,
      x1,
      y0: g.alto * 0.07 + i * paso,
      y1: g.alto * 0.07 + i * paso + paso * 0.72,
    })
  }
  return cajas
}
