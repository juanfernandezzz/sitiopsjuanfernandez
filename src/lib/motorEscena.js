...
export function crearMotor(canvas, opciones = {}) {
  ...
  function sembrar() {
    if (!escena) return
    const base = Math.min(ancho / 2.6, 430)
    ...
    cristales = []
    const nc = Math.round(Math.min(ancho / 2.6, 420) * escena.aire.escarcha)
    for (let i = 0; i < nc; i++) {
      cristales.push({
        x: Math.random() * ancho,
        y: Math.random() * alto,
    ...
    const nb = Math.round(Math.min(ancho / 3.2, 300) * (1 - ocupacion * 0.8))
    for (let i = 0; i < nb; i++) {
      brisa.push({
        x: Math.random() * ancho,
        y: Math.random() * alto,
    ...
    motas = []
    const nm = Math.round(Math.min(ancho / 4.5, 240) * escena.aire.polvo)
    for (let i = 0; i < nm; i++) {
      motas.push({
        x: Math.random() * ancho,
        y: Math.random() * alto,
  ...
  function construirVelo() {
    if (!paleta || !cajaTexto || ancho < 2 || alto < 2) {
      velo = null
      return
    }
    ...
    const m = Math.min(26, alto * 0.05)
    const d = Math.min(96, alto * 0.16, ancho * 0.1)

    const x0 = cajaTexto.x0 - m
    const x1 = cajaTexto.x1 + m
    const y0 = cajaTexto.y0 - m
    const y1 = cajaTexto.y1 + m
    ...
    // Perfil horizontal sobre toda la superficie.
    const gh = veloCtx.createLinearGradient(0, 0, ancho, 0)
    const paso = (v) => Math.max(0, Math.min(1, v / ancho))
    gh.addColorStop(0, nada)
    gh.addColorStop(paso(x0 - d), nada)
    gh.addColorStop(paso(x0), opaco)
    gh.addColorStop(paso(x1), opaco)
    gh.addColorStop(paso(x1 + d), nada)
    gh.addColorStop(1, nada)
    veloCtx.fillStyle = gh
    veloCtx.fillRect(0, 0, ancho, alto)

    // Perfil vertical, multiplicado sobre el anterior.
    veloCtx.globalCompositeOperation = 'destination-in'
    const gv = veloCtx.createLinearGradient(0, 0, 0, alto)
    const pasoV = (v) => Math.max(0, Math.min(1, v / alto))
    gv.addColorStop(0, 'rgba(0,0,0,0)')
    gv.addColorStop(pasoV(y0 - d), 'rgba(0,0,0,0)')
    gv.addColorStop(pasoV(y0), 'rgba(0,0,0,1)')
    gv.addColorStop(pasoV(y1), 'rgba(0,0,0,1)')
    gv.addColorStop(pasoV(y1 + d), 'rgba(0,0,0,0)')
    gv.addColorStop(1, 'rgba(0,0,0,0)')
    veloCtx.fillStyle = gv
    veloCtx.fillRect(0, 0, ancho, alto)
    veloCtx.globalCompositeOperation = 'source-over'
  ...
  function dibujarEstrellas(t) {
    ...
    if (trazos.length) {
      ...
      for (const [a, b] of trazos) {
        ctx.moveTo(a.x * ancho, a.y * alto * 0.86)
        ctx.lineTo(b.x * ancho, b.y * alto * 0.86)
      }
      ctx.stroke()
    }
  ...
  function dibujarAire(t, dt) {
    ...
    if (brisa.length) {
      ...
      for (const b of brisa) {
        ...
        b.f += 0.008 * paso
        b.x += (cx * 3.1 + escena.viento.deriva * b.v * (0.5 + escena.viento.fuerza * 2.4)) * paso
  ...
  function dibujarFugaz(t) {
    ...
    const dx = Math.cos(fugaz.ang) * fugaz.largo * ancho
    const dy = Math.sin(fugaz.ang) * fugaz.largo * alto
  ...
  return {
    ...
    destruir() {
      gotas = copos = cristales = motas = estrellas = trazos = brisa = aves = []
    },
  }
}
