...
### Geometrias auditadas

La primera version del auditor solo probaba apaisado. Por eso no detecto que
en una columna estrecha:

- el velo pasaba de cubrir el 64 por ciento del lienzo al 95 por ciento,
  porque se construia sobre el rectangulo que envuelve TODO el texto, y en
  vertical ese rectangulo es casi la seccion entera;
- la densidad de particulas caia a un tercio, porque el techo se calculaba
  sobre el ANCHO del lienzo y no sobre su area.

Correcciones: el velo se construye bloque a bloque de texto, con difuminado
reducido bajo 620 px de ancho para no sellar los huecos entre parrafos, y los
techos de particulas se calculan por area.

Medido tras la correccion, con 1351x420 y 390x1500:

| Escena | Movimiento escritorio | Movimiento movil | Contraste |
|---|---|---|---|
| Lluvia | 3.92% | 3.26% | 7.75 |
| Nublado | 4.38% | 8.15% | 7.70 |
| Despejado dia | 0.42% | 0.19% | 7.37 |
| Noche | 0.38% | 0.21% | 13.60 |
| Camanchaca | 2.59% | 0.35% | 8.01 |
| Nieve | 2.72% | 3.65% | 7.64 |
...
`scripts/auditar-dispositivos.mjs` recorre siete geometrias reales por ocho
escenas y verifica los cuatro invariantes en cada una. Ultima corrida: 56
combinaciones, 0 fallos.

| Dispositivo | Lienzo | Mov. minimo | Contraste minimo |
|---|---|---|---|
| iPhone SE | 375x1560 | ver salida | AAA |
| iPhone 14/15 | 390x1480 | ver salida | AAA |
| Pixel 8 | 412x1440 | ver salida | AAA |
| iPad vertical | 820x900 | ver salida | AAA |
| iPad apaisado | 1180x520 | ver salida | AAA |
| Laptop | 1366x440 | ver salida | AAA |
| Escritorio | 1920x460 | ver salida | AAA |

### Sobre el detector de bordes

La primera version marcaba como borde cualquier cambio de color sostenido.
Eso daba falsos positivos: el difuminado del velo al salir del lienzo produce
un gradiente de unos tres niveles por pixel, imperceptible a la vista, que el
detector contaba como raya de 180 px. Un borde es una DISCONTINUIDAD, no un
cambio: se detecta comparando la primera derivada con la de los pixeles
vecinos, de modo que un gradiente lineal, por pronunciado que sea, no dispara.

### Banda vegetal

Siluetas abstractas en el borde inferior que se doblan con el viento real.
No son una especie reconocible ni una silueta geografica: eso revelaria la
ubicacion inferida, que es justamente lo que la escena evita. Ancla la
composicion abajo y aporta movimiento permanente en la franja baja, que sin
ella quedaba vacia en pantallas estrechas.


## Auditoria geografica y estacional

`scripts/auditar-regiones.mjs` cubre las 16 regiones continentales por los 12
meses y las 24 horas. Se excluyen Rapa Nui y la Antartica Chilena por alcance.

La region y el mes no afectan a las particulas: afectan a la posicion del sol,
a la luna, al cielo estrellado y por lo tanto a la paleta. Todo eso es
numerico, asi que se verifica de forma exhaustiva sin rasterizar. El render se
audita aparte por dispositivo.

Ultima corrida: 82.944 combinaciones, 0 fallos.

  Contraste minimo:        7.77:1
  Altitud del astro:       de -89.4 a 89.0 grados
  Estrellas por noche:     21.9 promedio
  Noches con constelacion: 95 por ciento


## Verificacion en navegador real

### content-visibility: hipotesis descartada

Se sospechaba que `content-visibility: auto` con `contain-intrinsic-size:
auto 620px` en el envoltorio de la seccion hacia que el canvas se midiera con
el tamano de reemplazo en vez del real.

Reproducido en Chrome con la seccion a 6000 px del viewport y la contencion
activa:

| Elemento | Alto reportado |
|---|---|
| Envoltorio con content-visibility | 620 px (reemplazo) |
| Seccion | 781 px (real) |
| Capa del canvas | 781 px (real) |
| Contenido | 781 px (real) |
| Bloques de texto medidos | 14 de 14 con alto valido |

El componente mide `capa.getBoundingClientRect()`, que devuelve el alto REAL
aunque el ancestro reporte el de reemplazo. La hipotesis queda descartada.

### Hallazgo colateral sobre LazySection

El envoltorio reporta 620 px mientras su contenido mide 781: un desajuste de
161 px en escritorio. En movil, con la seccion apilada, la diferencia es de
mas de mil pixeles. `contain-intrinsic-size` mal calibrado provoca saltos de
scroll y estimaciones de altura erroneas. Es codigo preexistente, ajeno a este
ciclo, pero conviene corregirlo: el valor deberia ser `auto` a secas, que deja
al navegador recordar el tamano real tras el primer render.

### Rendimiento medido

Benchmark con la mezcla real de operaciones por cuadro, en el equipo de Juan
(4 nucleos):

| Geometria | ms por cuadro | Uso del presupuesto de 33 ms |
|---|---|---|
| Escritorio 1351x460 | 0.48 | 1 por ciento |
| Laptop 1366x440 | 0.48 | 1 por ciento |
| iPad 820x900 | 0.42 | 1 por ciento |
| Movil 390x1480 | 0.42 | 1 por ciento |
